...
PDFファイルをMarkdown形式に変換するシンプルで効率的なツールです。コマンドラインインターフェース(CLI)とウェブアプリケーションの両方で利用できます。
...
- `npm run web`: 開発モードでウェブサーバーを実行
- `npm run web:build`: ビルドして本番モードでウェブサーバーを実行
